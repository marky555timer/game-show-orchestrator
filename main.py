...
def main():
    ...
    print("                     Btn1 = EMERGENCY FORCE OVERRIDE (instant offline quiz, skips API)")
    print("                     Btn1/2 or Left Arrow = Back track, X- axis or Right Arrow = Next track")
    print("                     Btn4 = Auto-DJ ON/OFF toggle (default ON; auto-advances a track ~3s")
    print("                     before its end, using rekordbox.xml TotalTime or a 210s fallback)")
    print("                     Btn5 = Tempo Tap, Btn6 = Instant pull from pre-fetch queue")
    print("                     (10s cold-start timeout -> error buzz only, stays in DJ mode;")
    print("                     Btn1 force-overrides into an offline quiz if the cache never fills)")
